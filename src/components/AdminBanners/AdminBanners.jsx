...
const AdminBanners = () => {
  ...
  const fetchBanners = async () => {
    const res = await fetch('https://norte-production.up.railway.app/api/banners');
  ...
  const handleSubirImagen = async (e) => {
    ...
    try {
      ...
      const resClou = await fetch('https://api.cloudinary.com/v1_1/det2xmstl/image/upload', {
        method: 'POST',
        body: formData
      });
  ...
  const handleEliminar = async (id) => {
    ...
    await fetch(`https://norte-production.up.railway.app/api/banners/${id}`, { method: 'DELETE' });
    fetchBanners();
  };
    ...
    <div className={styles.adminContainer}>
      <div className={styles.headerAdmin}>
        ...
        <label className={styles.uploadBtn}>
          {subiendo ? <Loader2 className={styles.spin} /> : <Upload size={20} />}
          {subiendo ? 'Subiendo...' : 'Cargar desde Galería'}
          <input type="file" accept="image/*" onChange={handleSubirImagen} hidden disabled={subiendo} />
        </label>
      </div>
      ...
      <div className={styles.grid}>
        {banners.map(banner => (
          <div key={banner.id} className={styles.card}>
            ...
            <button onClick={() => handleEliminar(banner.id)}><Trash2 size={15} /></button>
          </div>
        ))}
      </div>
    </div>
